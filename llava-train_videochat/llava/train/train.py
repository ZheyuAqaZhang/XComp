...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    for i, source in enumerate(sources):
        ...
        for conv_id, conv in enumerate(source):
            ...
            if role in ["user", "system"]:
                ...
                target += encode_id
            if conv_id == 0:
                first_conv_length.append(len(input_id))
    ...
    return dict(
        ...
        first_conv_length=first_conv_length,  # list[int] (bs, )
    )
...
class LazySupervisedDataset(Dataset):
        ...
        if "{" in data_path and "}" in data_path:
            ...
            with open(data_path, "r") as file:
                ...
                for dataset in datasets:
                    ...
                    elif sampling_strategy == "first5" and sampling_number is not None:
                        cur_data_dict = cur_data_dict[sampling_number*4:sampling_number*5]
                        rank0_print(f"sampling_strategy={sampling_strategy}, {sampling_number*4}:{sampling_number*5}")
                    elif sampling_strategy == "first6" and sampling_number is not None:
                        cur_data_dict = cur_data_dict[sampling_number*5:sampling_number*6]
                        rank0_print(f"sampling_strategy={sampling_strategy}, {sampling_number*5}:{sampling_number*6}")
                    ...
                    extra_start_ratio = float(os.environ.get("EXTRA_PARAM_START_RATIO", 0))
                    extra_end_ratio = float(os.environ.get("EXTRA_PARAM_END_RATIO", 1))
                    if 'EXTRA_PARAM_SHUFFLE_SEED' in os.environ:
                        extra_seed = int(os.environ.get("EXTRA_PARAM_SHUFFLE_SEED", 0))
                        random.seed(extra_seed)
                        random.shuffle(cur_data_dict)
                        rank0_print(f"extra_seed={extra_seed}")
                    
                    if extra_start_ratio > 0 or extra_end_ratio < 1:
                        extra_start = int(len(cur_data_dict) * extra_start_ratio)
                        extra_end = int(len(cur_data_dict) * extra_end_ratio)
                        cur_data_dict = cur_data_dict[extra_start:extra_end]
                        rank0_print(f"extra_start_ratio={extra_start_ratio}, extra_end_ratio={extra_end_ratio}, {extra_start}:{extra_end}")
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(i, int):
            data_dict = dict(input_ids=data_dict["input_ids"][0], labels=data_dict["labels"][0], first_conv_length=data_dict["first_conv_length"][0])
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        batch['first_conv_length'] = [instance["first_conv_length"] for instance in instances]
...
def train(attn_implementation=None):
    ...
    if "EXTRA_PARAM_FREEZE_LAYER" in os.environ:
        ...
        if "EXTRA_PARAM_FORCE_COPY" in os.environ:
            rank0_print("Suppose freeze_layers weight is not initialized")
            model.condenser.freeze_layers.load_state_dict(model.get_model().layers[:n_freeze].state_dict())
            import time; time.sleep(5)
